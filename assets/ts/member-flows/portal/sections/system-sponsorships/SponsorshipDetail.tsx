...
export function SponsorshipDetail({
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const detailData = await getJson(`/api/v1/sponsorships/${encodeURIComponent(id)}`, sponsorshipResponseSchema);
  ...
  async function saveFields() {
    ...
    try {
      await patchJson(
        `/api/v1/sponsorships/${encodeURIComponent(id)}`,
        {
  ...
  async function advanceStage() {
    ...
    try {
      await patchJson(
        `/api/v1/sponsorships/${encodeURIComponent(id)}/stage`,
