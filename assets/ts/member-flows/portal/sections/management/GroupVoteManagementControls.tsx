...
/**
 * The vote's Settings facet: lifecycle first — open, close, cancel are what a
 * manager comes here for while a vote is live — then the schedule and text,
 * then who can see the result. One panel per concern, one submit per panel.
 */
export function GroupVoteSettings({
  groupId,
  vote,
  onChanged,
}: {
  ...
  return (
    // A named region: the management surface is what a screen-reader user
    // locates first when they arrive to run a vote.
    <section class="pk pk-stack" aria-label="Vote management">
      <ErrorAlert error={error} />
      <GroupVoteLifecycleActions groupId={groupId} vote={vote} onChanged={onChanged} />

      <Panel aria-labelledby={settingsHeadingId}>
        <PanelHeader id={settingsHeadingId} title="Wording and schedule" />
        <PanelBody>
      ...
      <Panel aria-labelledby={visibilityHeadingId}>
        <PanelHeader id={visibilityHeadingId} title="Who can see the result" />
        <PanelBody>
...
/**
 * The vote's Ballots facet: the identifiable ballot audit for managers. Its
 * own tab, so the query only runs when a manager opens it — the "Load
 * identifiable ballots" button this replaces was a fetch pretending to be a
 * disclosure control.
 */
export function GroupVoteBallots({ groupId, voteId }: { groupId: string; voteId: string }) {
  const base = `/api/v1/groups/${encodeURIComponent(groupId)}/votes/${encodeURIComponent(voteId)}`;
  return (
    <ApiDataTable
