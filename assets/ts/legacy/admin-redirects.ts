...
export const ADMIN_SPONSORSHIPS_REDIRECT_TARGET = "/portal/#/sponsors";
...
export const ADMIN_EVENTS_REDIRECT_TARGET = "/portal/#/events";
...
export function legacyAdminRedirectTarget(path: string): string | null {
  ...
  if (pathname === "/events") return ADMIN_EVENTS_REDIRECT_TARGET;
  const legacyProposalDetail = pathname.match(/^\/events\/([^/]+)\/proposal\/([^/]+)$/);
  if (legacyProposalDetail) {
    return `${ADMIN_EVENTS_REDIRECT_TARGET}/${encodeURIComponent(legacyProposalDetail[1])}/proposals/${encodeURIComponent(legacyProposalDetail[2])}`;
  }
  if (pathname.startsWith("/events/")) {
    return `${ADMIN_EVENTS_REDIRECT_TARGET}${pathname.slice("/events".length)}`;
  }
  return null;
}
