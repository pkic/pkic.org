...
function ReviewPanel({ review }: { review: ProposalReview }) {
  const recColour = { accept: "success", reject: "danger", "needs-work": "warning" }[review.recommendation] ?? "secondary";
  const reviewer = [review.reviewer_first_name, review.reviewer_last_name].filter(Boolean).join(" ") || review.reviewer_email || review.reviewer_user_id;
  return (
  ...
  return (
    <div class="d-flex gap-2 align-items-center border rounded p-2 mb-1">
      <div>
        ...
        {name !== speaker.email && <><br /><span class="text-muted small">{speaker.email}</span></>}
      </div>
...
function ProposalDetail({ proposal, slug, access }: { proposal: ProposalSummary; slug: string; access: ProposalAccess }) {
    ...
    <div class="p-3 bg-light border-top">
      <div class="row g-3">
        ...
        <div class="col-md-4">
          ...
          <dl class="small mb-0">
            <dt>Type</dt><dd>{proposal.proposal_type}</dd>
            <dt>Submitted</dt><dd>{fmt(proposal.submitted_at)}</dd>
            <dt>Updated</dt><dd>{fmt(proposal.updated_at)}</dd>
          </dl>
        </div>
      </div>
        ...
        <div class="mt-3 border-top pt-3">
          ...
          <form onSubmit={handleDecision} class="d-flex gap-2 align-items-end flex-wrap">
            <div>
              ...
              <select class="form-select form-select-sm" value={decisionStatus} onChange={(e) => setDecisionStatus((e.target as HTMLSelectElement).value)}>
            ...
            <div class="adm-flex-1">
              ...
              <input class="form-control form-control-sm" type="text" value={decisionNote} onInput={(e) => setDecisionNote((e.target as HTMLInputElement).value)} placeholder="Feedback for proposer…" />
            </div>
            <button type="submit" class="btn btn-sm btn-primary" disabled={savingDecision}>Save Decision</button>
            {decisionSaveStatus && <span class={`small ${decisionSaveStatus.startsWith("✓") ? "text-success" : "text-danger"}`}>{decisionSaveStatus}</span>}
          </form>
        </div>
      )}
    </div>
          ...
          <Table heads={["Title", "Proposer", "Type", "Status", "Decision", "Reviews", "Submitted", ""]} empty="No proposals found">
              {proposals.length > 0 && proposals.map((p) => {
                  ...
                  return (
                    <Fragment key={p.id}>
                      <tr>
                        <td class="adm-cell-title"><span class="small">{p.title}</span></td>
                        <td>
                          <span class="small">{proposer}</span>
                          {proposer !== p.proposer_email && <><br /><span class="text-muted small adm-cell-sub-email">{p.proposer_email}</span></>}
                        </td>
                        <td class="small">{p.proposal_type}</td>
                        <td><Badge status={p.status} /></td>
                        <td>{p.decision_status ? <Badge status={p.decision_status} /> : <span class="text-muted small">—</span>}</td>
                        <td class="mono">{p.review_count}</td>
                        <td class="mono small">{fmt(p.submitted_at)}</td>
                        <td>
                          <button class="btn btn-sm btn-outline-secondary" onClick={() => setExpandedId((prev) => prev === p.id ? null : p.id)}>
                      ...
                      {isExpanded && (
                        <tr>
                          <td colSpan={8} class="p-0">
                            <ProposalDetail proposal={p} slug={slug} access={access ?? { eventPermissions: [], canReview: false, canFinalize: false }} />
                          </td>
                        </tr>
                      )}
                    </Fragment>
