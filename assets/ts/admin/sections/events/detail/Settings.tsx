...
function SponsorTiersTab({ slug }: { slug: string }) {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  ...
  async function handleSave() {
    ...
    try {
      await api(`/api/v1/admin/events/${slug}/sponsor-tiers`, {
        method: "PUT",
        body: JSON.stringify({ tiers: tiers.filter((t) => t.tierName.trim()) }),
      });
      setSaveStatus("✓ Saved");
  ...
  return (
    <div>
      ...
      {tiers.map((t, i) => (
        <div class="row g-2 align-items-center mb-2" key={i}>
          <div class="col-sm-4">
            <input
              class="form-control form-control-sm"
              ...
              onInput={(e) =>
                setTiers((prev) =>
                  prev.map((x, j) => (j === i ? { ...x, tierName: (e.target as HTMLInputElement).value } : x)),
                )
              }
            />
          </div>
          <div class="col-sm-4">
            <div class="form-check">
              <input
                class="form-check-input"
                type="checkbox"
                checked={t.hasAttendeeDataAccess}
                onChange={(e) =>
                  setTiers((prev) =>
                    prev.map((x, j) =>
                      j === i ? { ...x, hasAttendeeDataAccess: (e.target as HTMLInputElement).checked } : x,
                    ),
                  )
                }
                id={`sponsor-tier-access-${i}`}
              />
              <label class="form-check-label small" for={`sponsor-tier-access-${i}`}>
      ...
      <button
        ...
        onClick={() => setTiers((prev) => [...prev, { tierName: "", hasAttendeeDataAccess: false }])}
      >
        + Add tier
      </button>
    </div>
