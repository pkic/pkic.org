...
function CurrencyOptions({ defaultCode }: { defaultCode: string }) {
  return (
    <>
      {CURRENCIES.map((c) => (
        <option key={c.code} value={c.code} selected={c.code === defaultCode}>
          {c.code.toUpperCase()} ({c.symbol}) — {c.name}
        </option>
      ))}
    </>
  );
...
function populateCurrencySelect(select: HTMLSelectElement, defaultCode: string): void {
  render(<CurrencyOptions defaultCode={defaultCode} />, select);
}
...
function PresetButtons({ currencyCode }: { currencyCode: string }) {
...
function renderPresets(container: HTMLElement, currencyCode: string): void {
  render(<PresetButtons currencyCode={currencyCode} />, container);
}
