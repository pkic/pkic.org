(function () {
  ...
  /*
   * `organization` / `independent` / `all` name a slice of the roll; a value
   * of `workingGroup:<slug>` narrows it to one group instead, which is what a
   * working group's own card counts.
   */
  function memberCountQuery(slice) {
    var group = slice.indexOf("workingGroup:") === 0 ? slice.slice("workingGroup:".length) : null;
    return group
      ? "workingGroup=" + encodeURIComponent(group) + "&group=organization"
      : "group=" + encodeURIComponent(slice);
  }

  function fetchMemberCount(group) {
    return fetch("/api/v1/members?" + memberCountQuery(group) + "&limit=1", {
  ...
  /*
   * Every count on the page, whichever slice it asks for.
   *
   * The mega menu asks for the organization and independent halves; the home
   * page's stats bar and the portal sign-in screen ask for the whole roll.
   * All three used to be separate — the menu read the API and the stats bar
   * counted YAML files at build time, so the same page could state two
   * different numbers of members (#8). They read one endpoint now, and the
   * distinct slices actually present on the page are the only ones fetched.
   */
  function hydrateMemberCounts() {
    if (memberCountsLoaded || memberCountsLoading) return;
    var slots = Array.prototype.slice.call(document.querySelectorAll("[data-member-count]"));
    if (slots.length === 0) return;
    memberCountsLoading = true;

    var groups = [];
    slots.forEach(function (slot) {
      var group = slot.getAttribute("data-member-count");
      if (groups.indexOf(group) === -1) groups.push(group);
    });

    Promise.all(groups.map(fetchMemberCount))
      .then(function (counts) {
        slots.forEach(function (slot) {
          var index = groups.indexOf(slot.getAttribute("data-member-count"));
          if (index !== -1) slot.textContent = String(counts[index]);
        });
  ...
  /*
   * A count outside the mega menu is on screen from the first paint, so it
   * cannot wait for a menu nobody may open. The menu's own counts stay lazy:
   * this only runs when the page has a slot the reader can already see. The
   * list endpoint is edge-cached, so the cost is one cached request.
   */
  function hydrateVisibleMemberCounts() {
    var visible = Array.prototype.slice.call(document.querySelectorAll("[data-member-count]")).some(function (slot) {
      return !slot.closest(".pkic-mega-panel");
    });
    if (visible) hydrateMemberCounts();
  }
  ...
  function resetMegaVisualState() {
    ...
    allTriggers.forEach(function (trigger) {
      trigger.querySelectorAll(".pkic-nav-link").forEach(function (link) {
  ...
  function openMega(triggerEl) {
    ...
    triggerEl.querySelectorAll(".pkic-nav-link").forEach(function (l) {
  ...
  if (sSubfilters) {
    sSubfilters.addEventListener("click", function (e) {
      ...
      sSubfilters.querySelectorAll(".pkic-facet-item").forEach(function (el) {
        var active =
          (el.dataset.kind === "tag" && el.dataset.value === currentTag) ||
  ...
  function renderResults() {
    ...
    var filtered = allResults.filter(function (d) {
      ...
      if (currentAuthor && !(d.filters && d.filters.author && d.filters.author.indexOf(currentAuthor) !== -1))
        return false;
      return true;
    });
    ...
    if (!filtered.length) {
      setResults(
        '<p class="pkic-search-hint">No results' +
          (currentType || currentTag || currentAuthor
            ? " matching these filters"
            : " for <strong>" + esc(currentQuery) + "</strong>") +
          "</p>",
      );
      return;
    }
      ...
      .map(function (d) {
        ...
        var metaHtml = meta.length
          ? '<div class="pkic-sr-meta">' +
            authors
              .map(function (a) {
                return '<span class="pkic-sr-tag pkic-sr-tag--author">' + esc(a) + "</span>";
              })
              .join("") +
            tags
              .map(function (t) {
                return '<span class="pkic-sr-tag">' + esc(t) + "</span>";
              })
              .join("") +
            "</div>"
          : "";
        return (
          '<a class="pkic-search-result-item" href="' +
          esc(d.url) +
          '">' +
          thumb +
          '<div class="pkic-sr-body">' +
          '<div class="pkic-sr-type" style="color:' +
          cfg.color +
          '">' +
          cfg.label +
          "</div>" +
          '<div class="pkic-sr-title">' +
          title +
          "</div>" +
          excerpt +
          metaHtml +
    ...
    setResults(
      '<p class="pkic-search-count">' +
        filtered.length +
        "\u00a0result" +
        (filtered.length !== 1 ? "s" : "") +
        "</p>" +
        '<div class="pkic-search-results-grid">' +
        cards +
        "</div>",
    );
  }
    ...
    sFilters.querySelectorAll(".pkic-filter-pill").forEach(function (pill) {
      ...
      var count = type
        ? allResults.filter(function (d) {
            return getTypeFromUrl(d.url) === type;
          }).length
        : allResults.length;
  ...
  function renderSubfilters() {
    ...
    var sectionResults = currentType
      ? allResults.filter(function (d) {
          return getTypeFromUrl(d.url) === currentType;
        })
      : allResults;
    ...
    sectionResults.forEach(function (d) {
      ((d.filters && d.filters.tag) || []).forEach(function (t) {
        tagCounts[t] = (tagCounts[t] || 0) + 1;
      });
      ((d.filters && d.filters.author) || []).forEach(function (a) {
      ...
      var rows = items
        .map(function (v) {
          ...
          return (
            ...
            '<span class="pkic-facet-name">' +
            esc(v) +
            "</span>" +
            '<span class="pkic-facet-count">' +
            counts[v] +
            "</span>" +
            "</button>" +
            "</li>"
        ...
        .join("");
      return (
        '<div class="pkic-facet-group">' +
        '<div class="pkic-facet-heading">' +
        heading +
        "</div>" +
        '<ul class="pkic-facet-list">' +
        rows +
        "</ul>" +
        "</div>"
      );
  ...
  async function runSearch(q) {
    ...
    try {
      ...
      allResults = await Promise.all(
        res.results.slice(0, 24).map(function (r) {
          return r.data();
        }),
      );
      updateFilterCounts();
