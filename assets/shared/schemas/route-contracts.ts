/** Compatibility barrel for route contracts grouped by API domain. */
export * from "./route-contracts-system";
export * from "./route-contracts-calendar";
export * from "./route-contracts-forms";
export * from "./route-contracts-events";
export * from "./route-contracts-headshots";
export * from "./route-contracts-proposal-management";
export * from "./route-contracts-proposal-presentations";
export * from "./route-contracts-event-registration-management";
export * from "./route-contracts-public-proposals";
export * from "./route-contracts-registrations";
export * from "./donation-webhook";
export * from "./email-webhook";
export * from "./group-forms";
export * from "./group-events";
