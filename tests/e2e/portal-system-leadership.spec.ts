...
const SYSTEM_LEADERSHIP_API = "/api/v1/system/leadership-positions";
...
test("permitted staff manage the public leadership roster through the System portal", async ({ page }) => {
  ...
  page.on("request", (request) => {
    ...
    if (pathname === SYSTEM_LEADERSHIP_API || pathname.startsWith(`${SYSTEM_LEADERSHIP_API}/`)) {
  ...
  const createResponse = page.waitForResponse(
    (response) => new URL(response.url()).pathname === SYSTEM_LEADERSHIP_API && response.request().method() === "POST",
  );
  ...
  const deleteResponse = page.waitForResponse(
    (response) =>
      new URL(response.url()).pathname.startsWith(`${SYSTEM_LEADERSHIP_API}/`) &&
  ...
  expect(systemRequests).toEqual(
    expect.arrayContaining([`GET ${SYSTEM_LEADERSHIP_API}`, `POST ${SYSTEM_LEADERSHIP_API}`]),
  );
