...
test("staff inspect delivery failures and edit a scheduled job interval", async ({ page }) => {
  ...
  await page.goto("/portal/#/settings/email-outbox");
  await page.getByRole("button", { name: "Status column options", exact: true }).click();
  const parent = page.getByRole("menu", { name: "Status column options", exact: true });
  await parent.getByRole("menuitem", { name: "Filter", exact: false }).click();
  const submenu = page.getByRole("menu", { name: "Filter", exact: true });
  await expect(submenu).toBeVisible();
  await expect(parent.getByRole("menuitemradio", { name: "Sort ascending", exact: true })).toBeVisible();
  const parentBox = await parent.boundingBox();
  const childBox = await submenu.boundingBox();
  expect(parentBox).not.toBeNull();
  expect(childBox).not.toBeNull();
  expect(childBox!.x >= parentBox!.x + parentBox!.width || childBox!.x + childBox!.width <= parentBox!.x).toBe(true);
