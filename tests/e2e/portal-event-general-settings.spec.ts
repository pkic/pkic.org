...
test("event settings require editing and keep the configured wall clock after saving and reloading", async ({
  ...
  const details = page
    .locator("section")
    .filter({ has: page.getByRole("heading", { name: "Event details", exact: true }) });
  await expect(details.locator("input,select,textarea")).toHaveCount(0);
  async function edit() {
    await page.getByRole("button", { name: "Event settings actions", exact: true }).click();
  ...
  const saved = page.waitForResponse(
    (response) => response.url().endsWith(`/events/${slug}/settings`) && response.request().method() === "PATCH",
  );
  await page.getByRole("button", { name: "Save changes", exact: true }).click();
  const response = await saved;
  expect(response.status(), await response.text()).toBe(200);
  expect(eventSettingsUpdateSchema.parse(response.request().postDataJSON()).startsAt).toBe("2027-06-10T07:00:00.000Z");
  await expect(details.locator("input,select,textarea")).toHaveCount(0);
  await page.reload();
