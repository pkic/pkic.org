...
import { dueWorkListQuerySchema, dueWorkListResponseSchema } from "../assets/shared/schemas/operations";
import { buildDueWorkQuery } from "../functions/_lib/services/due-work-read-model";
import { createAdminSession } from "./helpers/auth";
import { queryAll, seedEventAndAdmin } from "./helpers/context";
import { resetDb } from "./helpers/reset-db";

describe("operations due-work read model", () => {
  let token: string;
  ...
  beforeEach(async () => {
    ...
    token = await createAdminSession(env.DB, admin.id, "operations-due-work-token");
  });
  ...
  it("owns bucket filtering, sorting, counts, and pagination on the server", async () => {
    ...
    const firstPageResponse = await call(
      "/api/v1/operations/due-work?bucket=outbox&reminderLimit=1&outboxLimit=10&limit=1&offset=0&sort=dueAt",
    );
    expect(firstPageResponse.status).toBe(200);
    const firstPage = dueWorkListResponseSchema.parse(await firstPageResponse.json());
    ...
    const secondPage = dueWorkListResponseSchema.parse(
      await (
        await call(
          "/api/v1/operations/due-work?bucket=outbox&reminderLimit=1&outboxLimit=10&limit=1&offset=1&sort=dueAt",
        )
      ).json(),
    );
  ...
  it("bounds outbox candidate discovery before the read-model page query", async () => {
    ...
    const response = await call("/api/v1/operations/due-work?bucket=outbox&outboxLimit=2&limit=25&sort=dueAt");
    expect(response.status).toBe(200);
    const payload = dueWorkListResponseSchema.parse(await response.json());
  ...
  it("bounds reminder candidates before the global priority window", async () => {
    ...
    const response = await call("/api/v1/operations/due-work?bucket=reminders&reminderLimit=2&limit=25&sort=dueAt");
    expect(response.status).toBe(200);
    const payload = dueWorkListResponseSchema.parse(await response.json());
  ...
  it("shows a pending address only on the registration that owns its email change", async () => {
    ...
    const response = await call("/api/v1/operations/due-work?bucket=reminders&reminderLimit=10&limit=25&sort=dueAt");
    expect(response.status).toBe(200);
    const payload = dueWorkListResponseSchema.parse(await response.json());
  ...
  it("bounds historical cleanup candidate discovery with an explicit cleanup limit", async () => {
    ...
    const response = await call(
      "/api/v1/operations/due-work?bucket=cleanup&includeRetention=true&cleanupLimit=2&limit=25&sort=dueAt",
    );
    expect(response.status).toBe(200);
    const payload = dueWorkListResponseSchema.parse(await response.json());
  ...
  it("keeps the exact production projection on the bounded D1 candidate indexes", async () => {
    const query = dueWorkListQuerySchema.parse({
      bucket: "all",
    ...
    const statement = buildDueWorkQuery(env as any, "https://app.test", query);
    const plan = await env.DB.prepare(`EXPLAIN QUERY PLAN ${statement.sql}`)
      .bind(...statement.bindings)
      .all<{ detail: string }>();
    const details = plan.results.map((row) => row.detail).join("\n");

    expect(details).toContain("idx_email_outbox_due");
    expect(details).toContain("idx_proposal_speakers_speaker_invite_reminder_due");
    expect(details).toContain("idx_events_ends_at_id");
  });

  it("rejects an unallowlisted sort instead of interpolating it", async () => {
    expect((await call("/api/v1/operations/due-work?sort=recipient_email")).status).toBe(400);
  });
  ...
  it("treats malformed legacy outbox payload JSON as empty instead of failing the list", async () => {
    ...
    const response = await call("/api/v1/operations/due-work?bucket=outbox&outboxLimit=10");
    expect(response.status).toBe(200);
    const payload = dueWorkListResponseSchema.parse(await response.json());
    expect(payload.items).toEqual([expect.objectContaining({ title: "legacy@example.test" })]);
  });

  it("fails closed for authenticated staff without operations:read", async () => {
    ...
    expect((await call("/api/v1/operations/due-work")).status).toBe(403);

    await env.DB.prepare(
      `INSERT INTO permission_grants (id, user_id, permission, granted_by_user_id, created_at)
       VALUES (?, ?, 'operations:read', ?, datetime('now'))`,
    )
      .bind(crypto.randomUUID(), staffId, admin.id)
      .run();
    expect((await call("/api/v1/operations/due-work")).status).toBe(200);
  });
});
