...
/**
 * The roster of people who act for an organization, on the organization's own
 * page.
 *
 * It used to hang off "My profile", which put a list of coworkers on a page
 * about one person. The rules it enforces did not move with it: an ordinary
 * identity holder is offered no management, a contact may not end their own
 * identity or the primary contact's, and adding a coworker is a page of its
 * own rather than a layer over the directory.
 */
...
import {
  ADD_REPRESENTATIVE_SEGMENT,
  OrganizationRepresentatives,
} from "../../assets/ts/member-flows/portal/sections/OrganizationRepresentatives";
import { profile } from "../../assets/ts/member-flows/portal/state";

vi.mock("wouter/use-hash-location", () => ({ useHashLocation: () => ["/organizations/org", vi.fn()] }));
...
describe("an organization's representatives", () => {
  it("does not expose contact actions to an ordinary identity holder", () => {
    profile.value = currentProfile(false);
    void act(() => render(<OrganizationRepresentatives organizationId={organizationId} />, container));
  ...
  it("uses canonical lifecycle routes to update and end an identity while protecting self and primary contacts", async () => {
    ...
    void act(() =>
      render(
        <>
          <ConfirmDialogHost />
          <OrganizationRepresentatives organizationId={organizationId} />
        </>,
        container,
      ),
    );
    ...
    /*
     * Adding a coworker is a page of its own, at its own address under the
     * organization: nothing is layered over the directory, and the directory
     * is not underneath the form.
     */
      ...
      render(
        <>
          ...
          <OrganizationRepresentatives
            ...
            representativeSegment={ADD_REPRESENTATIVE_SEGMENT}
          />
        </>,
        container,
      ),
    ...
    // Back to the directory, which is where the rest of this walk happens.
    void act(() =>
      render(
        <>
          <ConfirmDialogHost />
