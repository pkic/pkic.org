...
import { api, apiCommand } from "../../assets/ts/member-flows/portal/api";
import { authStatus } from "../../assets/ts/member-flows/portal/state";
...
describe("portal API client", () => {
  ...
  it("keeps apiCommand success-only and rejects an undeclared payload", async () => {
    ...
    await expect(apiCommand("/api/v1/example", { method: "POST" })).rejects.toBeInstanceOf(z.ZodError);
  });
  ...
  it("clears the portal session after an unauthorized canonical API error", async () => {
    ...
    await expect(api("/api/v1/example", successResponseSchema)).rejects.toBeInstanceOf(ApiClientError);
