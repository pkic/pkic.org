...
import { EventAnalytics } from "../../assets/ts/member-flows/portal/sections/system-analytics/EventAnalytics";
...
function mount(initialTab?: string): HTMLElement {
  return mountNode(<EventAnalytics initialTab={initialTab} />);
}
...
describe("portal Event Analytics", () => {
  it("loads only the focused domain endpoint for the selected tab, never calls a legacy API, and no longer lists Donations", async () => {
    ...
    const tabLabels = Array.from(overview.querySelectorAll('nav[aria-label="Event analytics"] a')).map(
      (a) => a.textContent,
    );
...
describe("Event Analytics Overview on the design system", () => {
  it("names each region and table, and says an alarming count in words rather than in color", async () => {
     ...
     * analytics live under the domain they measure now, so the donations are
     * on /donations/analytics and email queue health is in the outbox, which
     * shows it in operational detail rather than as two numbers (#39).
     */
    const cardLabels = Array.from(container.querySelectorAll(".pk-stat-card")).map((card) => card.textContent);
    expect(cardLabels.some((label) => label?.includes("Failed Emails"))).toBe(false);
    expect(cardLabels.some((label) => label?.includes("Donations"))).toBe(false);
