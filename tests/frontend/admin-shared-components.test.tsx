...
import { EventScheduleFields } from "../../assets/ts/admin/components/EventScheduleFields";
import { RepresentativeContactSelect } from "../../assets/ts/admin/components/RepresentativeContactSelect";
...
describe("shared admin presentation components", () => {
  ...
  it("preserves representative selector semantics", () => {
  ...
  it("renders one settings shell for loading, errors, actions, and content", () => {
    const loading = mount(
      <AdminSettingsEditor loading error={null} description="Settings" actions={<button>Save</button>}>
        Content
      </AdminSettingsEditor>,
    );
    ...
    const ready = mount(
      <AdminSettingsEditor loading={false} error={null} description="Settings" actions={<button>Save</button>}>
        <span>Content</span>
      </AdminSettingsEditor>,
    );
  ...
  it("renders one active status tab and reports tab changes", () => {
    const onChange = vi.fn();
    const container = mount(<StatusTabs statuses={["pending", "approved"]} active="pending" onChange={onChange} />);
