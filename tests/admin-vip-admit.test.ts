...
async function seedInvite(_db: DatabaseLike, eventId: string, email: string): Promise<{ userId: string; inviteId: string }> {
...
describe("admin VIP admit", () => {
  ...
  it("admits waitlisted day attendance as capacity exempt and logs audit", async () => {
    ...
    const before = ((await queryAll<{ status: string }>(env.DB, 
    ...
    const audit = ((await queryAll<{ total: number }>(env.DB, 
