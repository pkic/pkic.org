...
## Phase 9 remediation pass — 2026-08-18

Implemented and verified both items of Phase 9 (§9.1–9.2) against baseline commit `b79ae11c37f8bad4f78601b479257770cea140b8`. **2 of 2 items complete**, both PASS with evidence below, across 7 commits (P9-02 landed first as a self-contained item; P9-01 landed as five sub-commits — google-groups retry/backoff, membership on-hold/EC bounding, sponsorship N+1/bounding, votes enqueue-only, then the shared registry wiring everything together — plus one same-pass typo fix).

### Checklist (extracted verbatim from Phase 9, IDs assigned this pass)

| ID | Location | Requirement (verbatim) | Plan (verbatim) |
| --- | --- | --- | --- |
| P9-01 | `functions/router.ts:103` (currently lines 93-118) [P2] | "Four independent workloads now run sequentially under the same 15-minute invocation, but only `runScheduledDueWork` participates in the existing time/subrequest/pass budget. An early failure prevents all later domains from running." Follow-up: "build one budgeted job registry the scheduled entrypoint dispatches through; do not add a Cloudflare Queue or Workflow in this pass; reuse `email_outbox`/`google_groups_sync_queue` instead of inventing new mechanisms." | "1. Own one budget at the dispatcher... Run sequentially with per-job `try/catch`... 2. No generic checkpoint rows... 3. Reuse existing durable abstractions: email producers enqueue `email_outbox` rows and stop calling `processOutboxByIdBackground` inside recipient loops... Improve `google_groups_sync_queue`... with bounded claim/retry/backoff/dead-letter semantics... 4. Bound the jobs themselves: `runOnHoldReminders`/`runEcWindowAutoApprove` need an indexed due predicate + stable `ORDER BY` + `LIMIT`...; sponsorship due-work needs its N+1 per-row lookups removed; vote notices should create outbox rows only... 5. Add tests demonstrating budget exhaustion and a mid-registry job failure neither lose nor duplicate work." |
| P9-02 | `functions/api/v1/admin/working-groups/[id]/meetings/[meetingId]/ics-files/index.ts:39` [P2] | "R2 upload and D1 metadata creation are not an atomic lifecycle: if `uploadIcsFile` fails, the object is orphaned; deletion has the inverse ordering and can leave an undeletable orphan." | "re-audit against `pkic.org#728`'s `presentation-archive.ts`... reuse its deterministic/idempotent object-key strategy... where the lifecycle semantics genuinely match; only build a second small service... otherwise scope it to `meeting-assets.ts` first... compensate the R2 put on D1 failure, or persist an explicit pending state before the put; make delete a state machine so a retry can finish safely." |

No ambiguous items in the two verbatim requirements themselves. Several judgment calls were made implementing the plan text — logged under Open Questions below.

### Baseline (before any change, commit `b79ae11c37f8bad4f78601b479257770cea140b8`)

- `git status`: clean except pre-existing untracked `csv/` and `prd/*.md` review docs (unrelated to this pass, not touched).
- `pnpm run typecheck` (backend/frontend/tools): clean.
- `pnpm run test:backend`: **973 passed, 1 skipped** (974), 0 failures.
- `pnpm run test:frontend`: **48 passed** (48).
- `pnpm run test:tools`: **51 passed, 1 failed** (52) — `tests/tools/migrate-members-importer.test.ts`'s idempotency smoke test times out at 120000ms; unrelated to Phase 9 (an importer/tooling test, no file this pass touches is anywhere near it), pre-existing.
...
- `pnpm run build`: succeeds, one pre-existing `INEFFECTIVE_DYNAMIC_IMPORT` warning, unrelated.
- `pnpm run check:max-lines`, `pnpm run check:filenames`: clean.
- `wrangler d1 migrations list pkic-db-preview --remote --env preview`: confirmed migrations `0035`–`0053` (the entire range this pass's schema changes fall in, `0036` and `0041`) are still **undeployed** to preview or production — safe to edit those two migration files in place per `AGENTS.md`'s "squash undeployed branch migrations into their final form" rule, no new `0054` migration needed.

### P9-02 implementation (`d4f61db7`)

`functions/_lib/services/meeting-calendar/admin-ics-files.ts`'s `uploadIcsFile` now takes the R2 `bucket` and raw `buffer`/`contentType` instead of a pre-computed `r2Key`, and performs the put itself: on a D1 insert failure after the put succeeds, it compensates by deleting the just-written object (`admin-ics-files.ts:37-49`) instead of leaving it orphaned. `deleteIcsFile` (`admin-ics-files.ts:102-133`) now takes `bucket` and deletes the R2 object *before* the D1 row — the reverse of the original ordering — because `R2Bucket#delete` on an already-missing key is a no-op, not an error: a throw partway through this function now always leaves the D1 row intact for a retry to find and finish, whereas the old D1-first ordering could delete the row and then fail the R2 delete, leaving an object no retry could ever reach again. Both the working-group and consortium ICS-file routes (which share this one service) were updated to pass the bucket through instead of doing their own `bucket.put`/`bucket.delete` calls, removing the duplicated ordering bug from both call sites at once.

**Deviation from the plan's literal suggestion, logged as a judgment call:** the plan suggested auditing `presentation-archive.ts` and potentially extracting a new `meeting-assets.ts` service. `presentation-archive.ts` turned out to be a pure read/zip-download service with no put/delete lifecycle at all (contrary to the plan's assumption) — nothing to reuse. `presentation-versions.ts` (a different, separate file) does have a comparable put+insert lifecycle, but is not named in the finding and has its own separate orphan-object exposure not part of this finding's scope. Per the plan's own stated criterion ("only build a second small service if at least two features share the same tested lifecycle, otherwise scope it to `meeting-assets.ts` first"), and since only one feature (ICS files, used by two routes sharing one existing service file) needed the fix, the fix was made in place in `meeting-calendar/admin-ics-files.ts` rather than extracting a new file — no second feature to justify a shared abstraction yet.

### P9-01 implementation

**Google Groups retry/backoff/dead-letter (`64eed0ad`):** `google_groups_sync_queue` gained an additive `next_attempt_at` column (folded into still-undeployed migration `0041`). `processGoogleGroupsSyncQueue` (`functions/_lib/services/google-groups.ts:270-291`) now keeps a failed row `pending` with `next_attempt_at` pushed out by exponential backoff (1 min, 2 min, 4 min... capped at 1 hour, `google-groups.ts:44-48`) instead of flipping straight to `failed`, up to `MAX_SYNC_ATTEMPTS` (5) tries, after which it dead-letters to `failed` and stops. `listPendingGoogleGroupsSync` (`google-groups.ts:87-93`) only claims rows whose backoff window has elapsed.

**Membership on-hold/EC bounding + enqueue-only (`5686df9d`):** `runOnHoldReminders` and `runEcWindowAutoApprove` (`functions/_lib/services/membership/scheduled-jobs.ts:111-244`) both gained a `limit` parameter (default 100) and `ORDER BY stage_entered_at ASC LIMIT ?`, backed by a new composite index `idx_member_applications_stage_entered_at` folded into still-undeployed migration `0036`. Both jobs, plus the Google Groups mailing-list-enrolled/wg-calendar-invite emails (`scheduled-jobs.ts:264-286`), stopped calling `processOutboxByIdBackground` per recipient inside their loops.

**Sponsorship N+1 removal + bounding + enqueue-only (`43a61ee5`):** `activeSponsorshipsWithRenewalDate` (`functions/_lib/services/sponsorship-scheduled-jobs.ts:64-88`) gained `ORDER BY sp.renewal_date ASC LIMIT ?` (backed by new index `idx_sponsorships_stage_renewal`, migration `0036`) and now selects `u.email AS assigned_to_email` directly from the join it already performed — the per-row `SELECT email FROM users WHERE id = ?` inside the loop is gone entirely (`sponsorship-scheduled-jobs.ts:114`). The three `queueEmail` call sites in the loop no longer call `processOutboxByIdBackground`.

**Votes enqueue-only (`f782abb5`):** `runVotesDueWork`'s delegate-notice loop (`functions/_lib/services/votes-scheduled-jobs.ts:31-49`) no longer calls `processOutboxByIdBackground` per recipient. `closeDueVotes` was already `LIMIT`-bounded (`functions/_lib/services/votes/closing.ts:103-118`), so no query-bounding change was needed there.

**Shared budgeted registry (`2f09d949`):** new `functions/_lib/services/scheduled-job-registry.ts` (86 lines) — `runScheduledJobRegistry` owns one shared deadline (`REMINDER_CRON_BUDGET_MS`, default 13 minutes, leaving headroom before the next 15-minute tick), runs jobs strictly sequentially (never concurrently), and wraps each job's own `run()` call in `try`/`catch` (`scheduled-job-registry.ts:63-82`) so one job's failure is recorded as that job's outcome and logged (`SCHEDULED_JOB_REGISTRY_ITEM_FAILED`) without suppressing later jobs — directly fixing "an early failure prevents all later domains from running." A job is skipped outright, not started, once less than its own declared `minRemainingMsToRun` is left in the shared budget (`SCHEDULED_JOB_REGISTRY_ITEM_SKIPPED_BUDGET`). `functions/router.ts:99-133`'s `REMINDER_CRON` branch now builds all four due-work jobs as `ScheduledJobDefinition`s and dispatches them through this registry instead of four bare sequential `await` calls with one shared `try` around the whole block.

**Plan step 2 ("no generic checkpoint rows"):** satisfied by construction, not by an explicit new mechanism — every bounded query added in this pass (on-hold, EC auto-approve, sponsorship due-work) uses deterministic `ORDER BY ... LIMIT`, same as votes' pre-existing `closeDueVotes`; no global last-processed-timestamp cursor was introduced anywhere.

### Validation for this pass

- `pnpm run typecheck` (backend/frontend/tools): clean after every commit.
- `pnpm exec eslint`/`pnpm exec prettier --check` on every touched file, `--max-warnings 0`: clean.
- `pnpm run test:backend`: **989 passed, 1 skipped** (990), 0 failures — the +16 delta over the 973-passed baseline is exactly the 16 new tests this pass added (2 in `meeting-calendar.test.ts`, 3 in `google-groups-sync.test.ts`, 2 in `membership-scheduled-jobs.test.ts`, 3 in `sponsorship-scheduled-jobs.test.ts`, 2 in the new `votes-scheduled-jobs.test.ts`, 4 in the new `scheduled-job-registry.test.ts`). No test that passed at baseline now fails.
- `pnpm run test:frontend`: **48/48**, identical to baseline (no frontend files touched).
- `pnpm run test:tools`: **51 passed, 1 failed** (52) — the exact same pre-existing `migrate-members-importer.test.ts` timeout as baseline, unrelated (no tooling file touched by this pass).
- `pnpm run build`: succeeds, same pre-existing `INEFFECTIVE_DYNAMIC_IMPORT` warning as baseline.
- `pnpm run lint`: same pre-existing 5833 `.venv` errors — every touched file individually lint-clean.
- `pnpm run check:max-lines`, `pnpm run check:filenames`: clean; largest touched/new file is `functions/_lib/services/membership/scheduled-jobs.ts` at 321 lines.
- `pnpm run test:e2e` was **not** run: this pass changes no browser-visible behavior, routing, or frontend code (confirmed via `git diff --stat` — every changed file is under `functions/`, `migrations/`, or `tests/`), so per `AGENTS.md`'s validation-cost guidance it is not a relevant gate here.

### Per-item evidence

| ID | Requirement (verbatim) | file:line satisfying it | Command run | Actual output | Verdict |
| --- | --- | --- | --- | --- | --- |
| P9-01 | "build one budgeted job registry the scheduled entrypoint dispatches through" + "Run sequentially with per-job `try/catch`" | `functions/_lib/services/scheduled-job-registry.ts:53-82`; wired at `functions/router.ts:99-133` | `pnpm exec vitest run --config vitest.config.ts tests/scheduled-job-registry.test.ts` | `Test Files  1 passed (1)` / `Tests  4 passed (4)` (mid-registry-failure isolation, budget-exhaustion skip, strict sequencing, empty-set cases) | **PASS** |
| P9-01 | "Improve `google_groups_sync_queue`... with bounded claim/retry/backoff/dead-letter semantics" | `functions/_lib/services/google-groups.ts:44-48,87-93,270-291`; `migrations/0041_membership_workflow.sql:110-122` (`next_attempt_at`) | `pnpm exec vitest run --config vitest.config.ts tests/google-groups-sync.test.ts` | `Test Files  1 passed (1)` / `Tests  8 passed (8)` (includes the 3 new retry/dead-letter/retry-success tests) | **PASS** |
| P9-01 | "`runOnHoldReminders`/`runEcWindowAutoApprove` need an indexed due predicate + stable `ORDER BY` + `LIMIT`" + "email producers enqueue `email_outbox` rows and stop calling `processOutboxByIdBackground` inside recipient loops" | `functions/_lib/services/membership/scheduled-jobs.ts:111-244`; `migrations/0036_applications_sponsorships_working_groups.sql:65-71` (index) | `pnpm exec vitest run --config vitest.config.ts tests/membership-scheduled-jobs.test.ts tests/membership-onboarding.test.ts` | `Test Files  2 passed (2)` / `Tests  24 passed (24)` (includes the 2 new LIMIT-bound tests + the 2 new `status === "queued"` assertions) | **PASS** |
| P9-01 | "sponsorship due-work needs its N+1 per-row lookups removed" + same `LIMIT`/enqueue-only requirements | `functions/_lib/services/sponsorship-scheduled-jobs.ts:64-88,111-114`; `migrations/0036...sql:157-163` (index) | `pnpm exec vitest run --config vitest.config.ts tests/sponsorship-scheduled-jobs.test.ts` | `Test Files  1 passed (1)` / `Tests  8 passed (8)` (includes the N+1-absence assertion via a `db.prepare`-call-capturing proxy, the LIMIT-bound test, and the `status === "queued"` test) | **PASS** |
| P9-01 | "vote notices should create outbox rows only, not send synchronously per recipient" | `functions/_lib/services/votes-scheduled-jobs.ts:31-49` | `pnpm exec vitest run --config vitest.config.ts tests/votes-scheduled-jobs.test.ts` | `Test Files  1 passed (1)` / `Tests  2 passed (2)` (new file — this job had zero prior test coverage) | **PASS** |
| P9-02 | "compensate the R2 put on D1 failure, or persist an explicit pending state before the put; make delete a state machine so a retry can finish safely" | `functions/_lib/services/meeting-calendar/admin-ics-files.ts:18-49,102-133` | `pnpm exec vitest run --config vitest.config.ts tests/meeting-calendar.test.ts` | `Test Files  1 passed (1)` / `Tests  20 passed (20)` (includes the new upload-orphan-compensation and delete-retry-safety failure-injection tests) | **PASS** |

**2 of 2 items complete.**

### Regressions vs. baseline

None. `pnpm run typecheck`, `pnpm run test:backend` (989 passed/1 skipped, up from 973/1 — all +16 are new tests), `pnpm run test:frontend` (48/48), `pnpm run test:tools` (51/1-pre-existing-failure/52, identical to baseline), `pnpm run lint` (5833 pre-existing `.venv` errors only), `pnpm run build`, `pnpm run check:max-lines`, `pnpm run check:filenames` all match baseline exactly or improve on it (only by adding passing tests) after all seven commits.

### Line-by-line diff review (edge cases, concurrency, resources, contract breaks, dead code)

- **Edge cases:** `uploadIcsFile`'s compensating `bucket.delete(r2Key).catch(() => {})` (`admin-ics-files.ts:47`) is deliberately swallowed — if the R2 delete itself also fails after a D1 failure, the object can still linger; this is a known, documented residual (compensation, not a real cross-service transaction, per the plan's own "do not pretend R2+D1 form one ACID transaction" framing), not silently hidden. `deleteIcsFile`'s R2-then-D1 ordering means a mid-function crash after a successful R2 delete but before the D1 row delete leaves a D1 row pointing at a now-missing object — a subsequent GET/download would 404 from R2 rather than silently break, and a retry of the same delete call is still safe (R2 delete of an already-gone key is a no-op).
- **Concurrency:** `google_groups_sync_queue`'s claim step (`listPendingGoogleGroupsSync`) is a bare `SELECT`, not a compare-and-set claim — two overlapping cron invocations could still both select and process the same row. This is **pre-existing behavior, not introduced or worsened by this pass** (confirmed by reading the baseline file at `b79ae11c`: no claim marker existed there either); Cloudflare Cron Triggers do not overlap the same schedule's invocations by default, so this is a latent risk rather than a currently-reachable bug, and fixing it was not named in the §9.1 finding (which is specifically about retry/backoff/dead-letter, not concurrent-claim safety) — flagged here rather than silently expanded into scope. The new job registry itself is provably sequential (`scheduled-job-registry.test.ts`'s "never starts two jobs concurrently" test).
- **Resources:** every previously-unbounded query (`runOnHoldReminders`, `runEcWindowAutoApprove`, `activeSponsorshipsWithRenewalDate`) now has a `LIMIT`; no new unbounded loops or queries were introduced anywhere in the diff.
- **Contract breaks:** `uploadIcsFile`'s exported signature changed (`(db, seriesId, expected, input)` → `(db, bucket, seriesId, expected, input)`) and `deleteIcsFile` gained a `bucket` parameter — both are internal service functions with exactly two call sites each (the working-group and consortium ICS-file routes), both updated in the same commit; confirmed via `grep -rln` that no other caller exists. `runOnHoldReminders`/`runEcWindowAutoApprove`/`runSponsorshipDueWork` gained trailing optional parameters with defaults — every existing call site (including every test) continues to compile and pass unmodified. No persisted-data-shape or API-response-shape changes anywhere; `next_attempt_at` is an additive, nullable column.
- **Dead/unreachable code:** none found — `pnpm exec eslint --max-warnings 0`'s `no-unused-vars` would have flagged any orphaned import or parameter; zero errors across every touched file. The `_env` parameter naming in `runOnHoldReminders`/`runVotesDueWork` (kept, unused, for call-site signature symmetry with their sibling due-work functions) is intentional, not dead code, and is lint-clean under this repo's `argsIgnorePattern: "^_"` convention.

### Security review (diff-only)

- **Injection:** every new/changed SQL statement uses `?` placeholders exclusively; `git diff` of the full pass grepped for template-literal SQL interpolation (`` `...${...}...` `` inside a `SELECT`/`INSERT`/`UPDATE`/`DELETE` string) found zero instances added by this pass.
- **AuthZ:** unchanged — no new endpoints, no new permission checks touched. The two ICS-file routes' existing `requireAdminFromRequest`/`requirePermission`/subtree-mount gating (from the Phase 4 pass) is untouched; only their internal call to the shared service changed shape.
- **Validation/output encoding:** no new user-rendered output; the Google Groups retry test's fake RSA keypair and fetch mocks are test-only scaffolding, never shipped.
- **Secrets/PII:** no new logging of secrets or PII. `SCHEDULED_JOB_REGISTRY_ITEM_FAILED`/`SCHEDULED_JOB_REGISTRY_ITEM_SKIPPED_BUDGET` log a job name and a generic error message, not request/user data. The google-groups retry test signs a real JWT with a synthetic, test-generated RSA key (never a real secret) purely to exercise the pre-existing `getServiceAccountAccessToken` code path against mocked `fetch` calls.
- **Crypto:** not touched by the diff itself; the test's `crypto.subtle.generateKey`/`exportKey` usage is test scaffolding exercising already-existing production RS256-signing code, not new crypto logic.
- **SSRF/redirects:** no new outbound requests introduced; the two outbound calls in `google-groups.ts` (OAuth token exchange, Directory API) are pre-existing and unchanged in this diff — only the post-failure state-machine around them changed.
- **New dependencies:** none — `git diff b79ae11c..HEAD -- package.json pnpm-lock.yaml` is empty; no audit run needed.
- **DoS:** every previously-unbounded scheduled query is now `LIMIT`-bounded; the new backoff mechanism caps retries at `MAX_SYNC_ATTEMPTS` (5) before dead-lettering, so a permanently-failing Google Groups operation cannot retry forever or accumulate unbounded queue growth. The shared registry's per-job budget check prevents one slow/stuck job from starving its siblings of their own time budget indefinitely (each is only started if enough of the shared budget remains).

No High/Critical/Medium findings. Nothing outstanding from this pass's own diff.

### Open questions and assumptions made

1. **`presentation-archive.ts` audit outcome contradicts the plan's assumption.** Logged in detail under P9-02 implementation above — the plan assumed a put/delete lifecycle to potentially reuse; the actual file has none. Read conservatively: audited as instructed, found nothing to reuse, did not force a reuse that doesn't fit, did not build the suggested new service since only one feature needed the fix.
2. **`deleteMeetingSeries` (cascading series deletion, `functions/_lib/services/meeting-calendar/admin-series.ts:138-149`) has the same D1-before-R2 ordering issue** the §9.2 finding describes, but for a *set* of files rather than one — it deletes all of a series' `meeting_ics_files` rows before the route handler's `Promise.allSettled` R2 deletes. This was **not** named in §9.2's finding text (`ics-files/index.ts:39`, the single-file endpoint) or its plan, and fixing a cascading multi-object delete safely (partial R2 failure mid-cascade) is a materially different, larger problem than the single-file fix delivered here. Left untouched per the task's "don't touch files unrelated to the current item" instruction — flagged here rather than silently left undiscovered.
3. **`presentation-versions.ts`'s `storePresentationFile`/`createPresentationVersion`** (a separate upload route, R2 put followed by a separate D1 insert call) has a structurally similar orphan-object exposure to the one §9.2 fixed for ICS files, but is a different feature entirely, not named in this finding, and out of scope for this pass.
4. **Google Groups queue claim-safety** (concurrent-invocation double-processing) — see "Concurrency" above under the diff review. Pre-existing, not part of §9.1's named scope (retry/backoff/dead-letter), not fixed in this pass.
5. **`REMINDER_CRON_BUDGET_MS` default (13 minutes) was chosen, not specified by the plan.** The plan says "own one budget at the dispatcher" without a number. 13 minutes leaves a ~2-minute safety margin before the next 15-minute cron tick, matching the existing `runScheduledDueWork`'s own internal 10-minute (`SCHEDULED_DUE_WORK_MAX_MS`) budget with room for the three sibling jobs to still get a turn even if due-work runs long. All four new/changed limit constants (`REMINDER_CRON_BUDGET_MS`, `SCHEDULED_ON_HOLD_REMINDER_LIMIT`, `SCHEDULED_EC_AUTO_APPROVE_LIMIT`, `SCHEDULED_SPONSORSHIP_DUE_WORK_LIMIT`) are environment-overridable, matching every other scheduled-job limit already in `config.ts`.

### Anything changed that was not in Phase 9

Nothing beyond the two named items' direct fixes and their own tests. `csv/` and `prd/*.md` remain untracked/uncommitted, unchanged by this pass.
