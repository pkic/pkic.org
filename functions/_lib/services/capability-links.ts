...
async function materializeString(
  ...
  const markers = Array.from(new Set(value.match(queuedMarkerPattern) ?? []));
  if (markers.length === 0) return value;

  await Promise.all(
    markers.map(async (marker) => {
      if (cache.has(marker)) return;
      ...
      cache.set(marker, token);
    }),
  );

  return markers.reduce((result, marker) => replaceQueuedMarker(result, marker, cache.get(marker)!), value);
}
...
async function materializeValue(
  ...
  if (typeof value === "string") return materializeString(db, signingSecret, value, cache);
  if (Array.isArray(value)) {
    return Promise.all(value.map((item) => materializeValue(db, signingSecret, item, cache)));
  }
  if (value && typeof value === "object") {
    const entries = await Promise.all(
      Object.entries(value).map(async ([key, item]) => [key, await materializeValue(db, signingSecret, item, cache)]),
    );
...
export async function materializeQueuedCapabilityLinks(
  ...
  if (!JSON.stringify(payload).includes(QUEUED_TOKEN_PREFIX)) return payload;
  ...
  return (await materializeValue(db, signingSecret, payload, new Map())) as Record<string, unknown>;
}
