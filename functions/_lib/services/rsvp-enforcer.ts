...
export async function runRsvpEnforcer(
  ...
  try {
    ...
    for (const b of bounces.results || []) {
      await db.batch([
        db
          .prepare(`UPDATE registrations SET status = 'cancelled', updated_at = datetime('now') WHERE id = ?`)
        ...
        db
          ...
          .bind(b.id),
        prepareAuditLog(db, "system", null, "cancelled_bounce", "registration", b.registration_id, {
          reason: "bounced_rsvp",
        }),
      ]);
      bouncesProcessed++;
    ...
    for (const w of warnings.results || []) {
      ...
      await db
        .prepare(`UPDATE calendar_rsvp_events SET warning_sent_at = datetime('now') WHERE id = ?`)
        .bind(w.id)
        .run();
    ...
    for (const action of pendingActions.results || []) {
      ...
      if (shouldExecute) {
        ...
        const newerAccepted = await db
          ...
          .bind(action.registration_id, action.id)
          .first();
        ...
        try {
          ...
          // Ignore JSON parse failures — use default settings
        }
