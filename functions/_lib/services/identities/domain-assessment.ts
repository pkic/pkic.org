import type { IdentityDomainAssessment } from "../../../../assets/shared/schemas/identity";
import {
  emailDomainOf,
...
interface DomainOrganizationRow {
...
function result(
  email: string,
  domain: string,
  outcome: IdentityDomainAssessment["outcome"],
  warning: string | null,
  organizationId: string | null = null,
): IdentityDomainAssessment {
  return {
    email,
    domain,
    outcome,
    mayCreateIdentity: outcome === "exact_claimed_match",
    warning,
    organizationId,
  };
}
...
export async function assessIdentityDomain(
  db: DatabaseLike,
  userId: string,
  emailInput: string,
): Promise<IdentityDomainAssessment> {
  ...
  if (isPersonalEmailDomain(domain)) {
    return result(
      ...
      "Personal or free email domains cannot establish an organization identity; an organization contact may invite you explicitly.",
    );
  }
  if (isDisposableEmailDomain(domain)) {
    return result(
      ...
      domain,
      "disposable_domain",
      "Disposable email domains cannot establish an organization identity; an organization contact may invite you explicitly.",
    );
  }
  ...
  if (!evidence?.verified_at) {
    return result(
      ...
      domain,
      "unverified_email",
      "Verify this email address before it can establish an organization identity.",
    );
  }

  const organizations = await all<DomainOrganizationRow>(
    db,
  ...
  if (blocked) {
    return result(
      ...
      "An organization contact blocked this identity; verified-domain activation remains disabled until an authorized manager creates a successor identity.",
      organizationId,
    );
  }
  return result(email, domain, "exact_claimed_match", null, organizationId);
}

/** Returns each exact verified claimed-domain match for reconciliation. */
export async function listVerifiedIdentityDomainMatches(
  db: DatabaseLike,
  userId: string,
): Promise<Array<{ email: string; domain: string; organizationId: string }>> {
  const rows = await all<{ email: string; domain: string; organization_id: string }>(
    db,
  ...
  return rows
    ...
    .map((row) => ({ email: row.email, domain: row.domain, organizationId: row.organization_id }));
}
