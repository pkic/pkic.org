...
export interface SponsorshipReadModelRow {
  id: string;
  sponsor_type: string;
...
export async function listSponsorships(
...
): Promise<{ sponsorships: SponsorshipReadModelRow[]; total: number }> {
  const { where, values } = buildSponsorshipsWhere(filters);
  ...
  const { rows: sponsorships, total } = await queryPage<SponsorshipReadModelRow>(db, {
    sql: `${SPONSORSHIP_SELECT} ${where}`,
    bindings: values,
    orderBy,
    limit: filters.limit,
...
export interface SponsorshipCompanyRow {
  key: string;
  label: string;
  website: string | null;
...
export async function listSponsorshipCompanies(
...
): Promise<{ companies: SponsorshipCompanyRow[]; total: number }> {
  const { where, values } = buildSponsorshipsWhere(filters);
  const groupedCte = `
    WITH grouped AS (
      SELECT
  ...
  const { rows: companies, total } = await queryPage<SponsorshipCompanyRow>(db, {
    sql: `${groupedCte}
...
export async function getSponsorship(db: DatabaseLike, id: string): Promise<SponsorshipReadModelRow | null> {
  return first<SponsorshipReadModelRow>(db, `${SPONSORSHIP_SELECT} WHERE sp.id = ?`, [id]);
}

export function toApiSponsorship(row: SponsorshipReadModelRow) {
  return {
    id: row.id,
