import type { AdminStatsResponse } from "../../../assets/shared/schemas/admin-analytics";
import { adminStatsResponseSchema } from "../../../assets/shared/schemas/admin-analytics";
import { all, first } from "../db/queries";
import type { DatabaseLike } from "../types";

/**
 * Builds aggregate platform statistics for dashboards and reporting. Keeping
 * this read model out of the route prevents HTTP/auth concerns from becoming
 * coupled to its SQL projections.
 */
export async function getAdminPlatformStats(db: DatabaseLike): Promise<AdminStatsResponse> {
  const [
    ...
    all<{ slug: string; name: string; confirmed: number; total: number }>(
      ...
      `SELECT
         e.slug, e.name,
         COUNT(DISTINCT CASE WHEN r.status = 'registered' THEN r.id END) AS confirmed,
