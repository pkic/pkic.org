...
import { sponsorsDisplayRouteSchema, sponsorsListRouteSchema } from "../../../../assets/shared/schemas/public-sponsors";
import { openApiRoute } from "../../../_lib/openapi/route";

const PUBLIC_CACHE_CONTROL = "public, max-age=300, s-maxage=900, stale-while-revalidate=60";

export const SponsorsGet = openApiRoute(sponsorsListRouteSchema, async (c: any, data) => {
  const { eventSlug, eventName, level, minWeight, limit, offset, q, sort } = data.query;

  const body = await listPublicSponsors(c.env.DB, { eventSlug, eventName, level, minWeight, limit, offset, q, sort });
...
export const SponsorsDisplayGet = openApiRoute(sponsorsDisplayRouteSchema, async (c: any, data) => {
  const { eventSlug, eventName, level, minWeight, limit, offset, q, sort } = data.query;
  const body = await listPublicSponsorDisplay(c.env.DB, {
    eventSlug,
    eventName,
    level,
    minWeight,
