...
| 4.4a | Does the CBOM bind each cryptographic-asset entry to the component that contains it via a standardised product identifier — Common Platform Enumeration (CPE) or Package URL (purl) — so that algorithm-level findings can be correlated with CVE feeds and asset inventories? | In CycloneDX (cryptography extension), this is typically a `cpe` or `purl` field on the component referenced by each `cryptographic-asset`. Sample at least five cryptographic-asset entries and record, for each, the referenced component identifier and whether a CPE or purl is present. Library-level entries that are not linked to a component identifier cannot be matched against the NIST National Vulnerability Database — record each gap individually. The top-level product component must carry the same CPE identifier as the one recorded for the [certification application](/wg/pqc/pqcmm/assessment/certification/#product-identifiers). |
